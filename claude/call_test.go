...
func TestCall(t *testing.T) {
	...
	message, _, err := client.Call("claude-sonnet-4-6", "", []Message{
		{
...
func TestCallStream(t *testing.T) {
	client := newTestClient()
	resMessages, _, err := client.CallStream("claude-sonnet-4-6", "", []Message{
		{
			Role:    ClaudeMessageRoleUser,
			Content: SingleStringMessage("你好"),
		},
...
func TestCallStreamWithTools(t *testing.T) {
	...
	message, _, err := client.CallStream("claude-sonnet-4-6", "", []Message{
		{
			Role:    ClaudeMessageRoleUser,
			Content: SingleStringMessage("大连天气怎么样"),
		},
